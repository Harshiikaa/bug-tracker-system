...
// export default function TesterDashboard() {
//   const { token } = useAuth();
//   const { createBug, getBugsForDashboard } = useBugs(token);
//   const params = useParams();
...
//     setError(null);
//     try {
//       const response = await getBugsForDashboard();
//       if (response.success && response.data) {
//         const fetchedBugs = response.data;
//         if (bugId) {
//           const filteredBug = fetchedBugs.find((bug: Bug) => bug._id === bugId);
//           setBugs(filteredBug ? [filteredBug] : []);
//           if (!filteredBug) {
//             setError('Bug not found or unauthorized access');
//           }
//         } else {
//           setBugs(fetchedBugs);
//         }
//       } else {
//         setBugs([]);
//         setError(response.message || 'No bugs found.');
//       }
//     } catch (err) {
//       setBugs([]);
...
//       setLoading(false);
//     }
//   }, [getBugsForDashboard]); // Removed bugId from dependencies

//   useEffect(() => {
//     fetchBugs();
//   }, [fetchBugs, bugId]); // Include bugId to handle URL changes
...
//       } else {
//         setError(
//           response.errors?.join(', ') || response.message || 'Bug creation failed'
//         );
//       }
//     } catch (err) {
//       setError(err instanceof Error ? err.message : 'Bug creation failed');
